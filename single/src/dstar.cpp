 ...
 */
const list<state>& Dstar::getPath() const {
	return path;
}
...
double Dstar::eightCondist(state a, state b) {
	//    double temp;
	double min = std::abs(a.x - b.x);
	double max = std::abs(a.y - b.y);
	if (min > max) {
		double temp = min;
		min = max;
		max = temp;
 ...
 *    iterate through it.
 */
int Dstar::computeShortestPath(double ms) {
	...
	while (not openList.empty()) {
		s_start = calculateKey(s_start);
		if (s_start <= openList.top() and getRHS(s_start) == getG(s_start))
			break;
		state u;
		timer t;
		t.start();
 ...
 */
bool Dstar::replan(double ms) {
	//fprintf(stderr, "replan: %d\n", cellHash.size());

	path.clear();

	//added by pablo urcola
	if (occupied(s_goal) || occupied(s_start))
		return false;

	int res = computeShortestPath(ms);
